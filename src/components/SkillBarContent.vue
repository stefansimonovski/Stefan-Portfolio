  ...
  <section class="flex sm:flex-row flex-col lg:pt-10">
    ...
    <div class="flex flex-row sm:flex-col mt-4 ml-[auto] sm:ml-0">
      <button
        ...
        @click="
          () => {
            currentSelectedTechnology(value)
...
export default {
  ...
  data() {
    return {
      ...
      selectedTech: this.currentSelectedTechnology('frontend'),
  ...
  mounted() {
    this.currentSelectedTechnology('frontend')
    this.setDefaultSelectLang(0)
  },

  methods: {
    currentSelectedTechnology(type) {
      this.selectedTech = type
    },
